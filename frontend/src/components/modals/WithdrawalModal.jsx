...
// import MessageModal from "./MessageModal";
// import { useAuth } from "../../context/AuthContext";
// import { Wallet, Zap, Layers, Users, Trophy, X, UserCog, Eye, EyeOff, Info } from "lucide-react"; 
// import { Link } from "react-router-dom"; 
...
//         if (amt > 0) {
//             if (!lvl.isDirectMet && !isPromo) {
//                 throw new Error(`Please complete Total ${lvl.reqDirects} Directs to withdraw from Community Lvl ${lvl.level}.`);
//             }
//             if (!isPromo && amt > lvl.available) throw new Error(`Insufficient funds in Level ${lvl.level} Pool.`);
...
//       // 🔥 NAYA FEATURE: Detailed Breakdown dikhana Popup mein
//       const rep = response.data.report; 
//       if (rep) {
//           const breakdownMessage = `
//             WITHDRAWAL REPORT (Team Size: ${rep.teamSizeTracked}):
//             Requested: $${rep.totalRequested.toFixed(2)}
//             Fee Deducted (10%): $${rep.totalFeeDeducted.toFixed(2)}
//             ------------------------
//             ✅ To USDT Address: $${rep.totalNetUSDT.toFixed(2)}
//             ✅ To Top-up Wallet: $${rep.totalToTopupWallet.toFixed(2)}
//             🔥 Flushed (System): $${rep.totalFlushed.toFixed(2)}
//           `;
//           // Pehle details dikhao, fir success modal
//           showMessage("Withdrawal Calculation", breakdownMessage, "info");
//       }
...
//             // 🔥 NAYA FEATURE: Agar breakdown ho to naye lines theek se dikhe
//             message={<pre className="font-mono text-[10px] sm:text-xs text-left p-2 bg-slate-50 border border-slate-200 rounded-lg">{messageModal.message}</pre>} 
