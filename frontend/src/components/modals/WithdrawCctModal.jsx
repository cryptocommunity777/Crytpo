...
// import SuccessModal from "./SuccessModal";
// import { useAuth } from "../../context/AuthContext"; 
// import { Download, X, Wallet, AlertCircle } from "lucide-react"; 
...
//   cctStakingLevelIncome = 0,
//   monthlySalaryWallet = 0,
//   // 🔥 NAYE COMPOUND WALLETS PROPS
//   cctCompoundIncome = 0,
//   cctCompoundDirectIncome = 0,
//   cctCompoundLevelIncome = 0
// }) => {
//   const { user: loggedInUser } = useAuth();
...
//   // 🔥 States (New Compound Wallets)
//   const [compoundAmount, setCompoundAmount] = useState("");
//   const [compoundDirectAmount, setCompoundDirectAmount] = useState("");
//   const [compoundLevelAmount, setCompoundLevelAmount] = useState("");
...
//   // 🔥 Total Amount Calculation (Sab add hoga)
//   const totalAmount = 
//     (Number(roiAmount) || 0) + 
//     (Number(directAmount) || 0) + 
//     (Number(levelAmount) || 0) + 
//     (Number(salaryAmount) || 0) + 
//     (Number(compoundAmount) || 0) + 
//     (Number(compoundDirectAmount) || 0) + 
//     (Number(compoundLevelAmount) || 0);
...
//     if (Number(directAmount) > Number(cctStakingDirectIncome)) return showMessage("Error", "❌ Insufficient Staking Direct balance.", "error");
//     if (Number(levelAmount) > Number(cctStakingLevelIncome)) return showMessage("Error", "❌ Insufficient Staking Level balance.", "error");
//     if (Number(salaryAmount) > Number(monthlySalaryWallet)) return showMessage("Error", "❌ Insufficient Monthly Salary balance.", "error"); 
    
//     // 🔥 New Compound Balances checks
//     if (Number(compoundAmount) > Number(cctCompoundIncome)) return showMessage("Error", "❌ Insufficient Compound ROI balance.", "error");
//     if (Number(compoundDirectAmount) > Number(cctCompoundDirectIncome)) return showMessage("Error", "❌ Insufficient Compound Direct balance.", "error");
//     if (Number(compoundLevelAmount) > Number(cctCompoundLevelIncome)) return showMessage("Error", "❌ Insufficient Compound Level balance.", "error");
    
//     const items = [];
//     if (Number(roiAmount) > 0) items.push({ source: "cct_staking", amount: Number(roiAmount) });
//     if (Number(directAmount) > 0) items.push({ source: "cct_direct", amount: Number(directAmount) });
//     if (Number(levelAmount) > 0) items.push({ source: "cct_level", amount: Number(levelAmount) });
//     if (Number(salaryAmount) > 0) items.push({ source: "monthly_salary", amount: Number(salaryAmount) }); 
    
//     // 🔥 New items push
//     if (Number(compoundAmount) > 0) items.push({ source: "cct_compound", amount: Number(compoundAmount) });
//     if (Number(compoundDirectAmount) > 0) items.push({ source: "cct_compound_direct", amount: Number(compoundDirectAmount) });
//     if (Number(compoundLevelAmount) > 0) items.push({ source: "cct_compound_level", amount: Number(compoundLevelAmount) });
...
//               {/* === OLD STAKING WALLETS (50-50 & 10% Fee) === */}
//               <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1 mb-[-4px]">Legacy Wallets (50/50 Split)</div>
              
//               <div className="flex flex-col gap-1.5 bg-slate-50 border border-slate-200 rounded-xl p-3 shadow-sm">
//                  <div className="flex justify-between items-center">
//                     <span className="text-black text-[10px] uppercase tracking-wider font-bold flex items-center gap-1">
//                       <Wallet size={12} className="text-emerald-500"/> Staking ROI
//                     </span>
//                     <span className="text-xs font-black text-emerald-600 font-mono">Bal: {Number(cctStakingIncome || 0).toFixed(2)}</span>
//                  </div>
//                  <input type="number" placeholder="Enter Amount" value={roiAmount} onChange={(e) => setRoiAmount(e.target.value)} className="w-full bg-white text-slate-800 rounded-lg px-3 py-2 outline-none font-mono text-xs border border-slate-200 focus:border-emerald-400 focus:ring-1 focus:ring-emerald-200" />
//               </div>
...
//                     <span className="text-xs font-black text-blue-600 font-mono">Bal: {Number(cctStakingDirectIncome || 0).toFixed(2)}</span>
//                  </div>
//                  <input type="number" placeholder="Enter Amount" value={directAmount} onChange={(e) => setDirectAmount(e.target.value)} className="w-full bg-white text-slate-800 rounded-lg px-3 py-2 outline-none font-mono text-xs border border-slate-200 focus:border-blue-400 focus:ring-1 focus:ring-blue-200" />
//               </div>
...
//                     <span className="text-xs font-black text-purple-600 font-mono">Bal: {Number(cctStakingLevelIncome || 0).toFixed(2)}</span>
//                  </div>
//                  <input type="number" placeholder="Enter Amount" value={levelAmount} onChange={(e) => setLevelAmount(e.target.value)} className="w-full bg-white text-slate-800 rounded-lg px-3 py-2 outline-none font-mono text-xs border border-slate-200 focus:border-purple-400 focus:ring-1 focus:ring-purple-200" />
//               </div>
...
//                     <span className="text-xs font-black text-orange-600 font-mono">Bal: {Number(monthlySalaryWallet || 0).toFixed(2)}</span>
//                  </div>
//                  <input type="number" placeholder="Enter Amount" value={salaryAmount} onChange={(e) => setSalaryAmount(e.target.value)} className="w-full bg-white text-slate-800 rounded-lg px-3 py-2 outline-none font-mono text-xs border border-slate-200 focus:border-orange-400 focus:ring-1 focus:ring-orange-200" />
//               </div>

//               {/* === NEW COMPOUNDING WALLETS (100% Withdraw, No Fee) === */}
//               <div className="text-[10px] font-black text-blue-500 uppercase tracking-widest pl-1 mt-4 mb-[-4px]">Compounding Wallets (100% Payout, 0% Fee)</div>

//               <div className="flex flex-col gap-1.5 bg-blue-50/50 border border-blue-100 rounded-xl p-3 shadow-sm">
//                  <div className="flex justify-between items-center">
//                     <span className="text-black text-[10px] uppercase tracking-wider font-bold flex items-center gap-1">
//                       <Wallet size={12} className="text-blue-500"/> Compound Income
//                     </span>
//                     <span className="text-xs font-black text-blue-600 font-mono">Bal: {Number(cctCompoundIncome || 0).toFixed(2)}</span>
//                  </div>
//                  <input type="number" placeholder="Enter Amount" value={compoundAmount} onChange={(e) => setCompoundAmount(e.target.value)} className="w-full bg-white text-slate-800 rounded-lg px-3 py-2 outline-none font-mono text-xs border border-slate-200 focus:border-blue-400 focus:ring-1 focus:ring-blue-200" />
//               </div>

//               <div className="flex flex-col gap-1.5 bg-blue-50/50 border border-blue-100 rounded-xl p-3 shadow-sm">
//                  <div className="flex justify-between items-center">
//                     <span className="text-black text-[10px] uppercase tracking-wider font-bold flex items-center gap-1">
//                       <Wallet size={12} className="text-blue-500"/> Compound Direct
//                     </span>
//                     <span className="text-xs font-black text-blue-600 font-mono">Bal: {Number(cctCompoundDirectIncome || 0).toFixed(2)}</span>
//                  </div>
//                  <input type="number" placeholder="Enter Amount" value={compoundDirectAmount} onChange={(e) => setCompoundDirectAmount(e.target.value)} className="w-full bg-white text-slate-800 rounded-lg px-3 py-2 outline-none font-mono text-xs border border-slate-200 focus:border-blue-400 focus:ring-1 focus:ring-blue-200" />
//               </div>

//               <div className="flex flex-col gap-1.5 bg-blue-50/50 border border-blue-100 rounded-xl p-3 shadow-sm">
//                  <div className="flex justify-between items-center">
//                     <span className="text-black text-[10px] uppercase tracking-wider font-bold flex items-center gap-1">
//                       <Wallet size={12} className="text-blue-500"/> Compound Level
//                     </span>
//                     <span className="text-xs font-black text-blue-600 font-mono">Bal: {Number(cctCompoundLevelIncome || 0).toFixed(2)}</span>
//                  </div>
//                  <input type="number" placeholder="Enter Amount" value={compoundLevelAmount} onChange={(e) => setCompoundLevelAmount(e.target.value)} className="w-full bg-white text-slate-800 rounded-lg px-3 py-2 outline-none font-mono text-xs border border-slate-200 focus:border-blue-400 focus:ring-1 focus:ring-blue-200" />
//               </div>

//               {/* Live Total Display */}
//               <div className="flex justify-between items-center p-2 px-3 bg-emerald-50 border border-emerald-200 rounded-xl mt-2 sticky bottom-0">
//                  <span className="text-emerald-800 text-[11px] font-black uppercase tracking-widest">Total Withdrawal</span>
//                  <span className={`text-base font-black font-mono ${totalAmount > 0 && totalAmount % 10 === 0 ? 'text-emerald-600' : 'text-red-500'}`}>
//                     {totalAmount} CCT
//                  </span>
...
//                  {/* 🔥 LEADER ROLE CHECK: Added proper message for Leaders 🔥 */}
//                  {loggedInUser?.role === 'leader' ? (
//                     <div className="p-3 bg-red-50 border border-red-200 rounded-lg flex items-center justify-center gap-2">
//                       <AlertCircle size={16} className="text-red-500" />
//                       <span className="text-xs font-bold text-red-600">
//                         Withdrawals are restricted for Leader accounts.
//                       </span>
//                     </div>
//                  ) : (
//                     <>
