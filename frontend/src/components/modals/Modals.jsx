...
//                {/* 1. USDT BEP20 TRANSFER BUTTON 🔥 (YE WALA MISSING THA) */}
//               <button
//                 onClick={() => {
...
//               {/* 2. TOP-UP WALLET TRANSFER BUTTON */}
//               <button
//                 onClick={() => {
//                   closeModal("showTransferSelection");
...
import TransferCctModal from "./TransferCctModal";
import UsdtBep20TransferModal from "./UsdtBep20TransferModal";
// 🔥 NAYA: Reinvest wala naya modal import kiya
import ReinvestCompoundModal from "./ReinvestCompoundModal";
import { X, DollarSign, Coins, Wallet } from "lucide-react"; 
...
const Modals = ({ user, modalState, setModalState, setUser }) => {
  ...
  return (
    <>
      ...
      {modalState.showDeposit && (
        <DepositModal isOpen={modalState.showDeposit} onClose={() => closeModal("showDeposit")} userId={user.userId} walletAddress={adminWalletAddress || ""} onDepositSuccess={(amount) => { closeModal("showDeposit"); setSuccessData({ userId: user.userId, amount, type: "deposit" }); }} />
      )}

      {modalState.showWalletTransfer && (
        <WalletTransferModal onClose={() => closeModal("showWalletTransfer")} onSuccess={(userId, amount) => setSuccessData({ userId, amount, type: "transfer" }) } />
      )}

      {modalState.showUsdtBep20Transfer && (
        <UsdtBep20TransferModal onClose={() => closeModal("showUsdtBep20Transfer")} onSuccess={(userId, amount) => setSuccessData({ userId, amount, type: "transfer" }) } />
      )}

      {modalState.showCctTransfer && (
        <TransferCctModal onClose={() => closeModal("showCctTransfer")} cctBalance={user?.cctBalance || 0} onSuccess={(userId, amount) => setSuccessData({ userId, amount, type: "transfer" }) } />
      )}

      {modalState.showWithdrawalModal && (
        <WithdrawalModal userId={user.userId} onClose={() => closeModal("showWithdrawalModal")} onSuccess={(updatedUser, amount) => updateUserAndSuccess(updatedUser, amount, "withdrawal") } />
      )}

      {modalState.showInstantWithdraw && (
        <InstantWithdrawModal userId={user.userId} onClose={() => closeModal("showInstantWithdraw")} onSuccess={(updatedUser, amount) => updateUserAndSuccess(updatedUser, amount, "instant-withdraw") } />
      )}

      {modalState.showTopUpForm && (
        <TopUpModalWithInput onClose={() => closeModal("showTopUpForm")} onTopUpSuccess={(updatedUser, amount) => updateUserAndSuccess(updatedUser, amount, "topup") } />
      )}

      {modalState.showCreditToWallet && (
        <CreditToWalletModal userId={user.userId} onClose={() => closeModal("showCreditToWallet")} onSuccess={(updatedUser, amount) => updateUserAndSuccess(updatedUser, amount, "credit") } />
      )}

      {/* 🔥 NAYA: REINVEST COMPOUND INCOME MODAL */}
      {/* {modalState.showReinvestCompound && (
        <ReinvestCompoundModal 
          onClose={() => closeModal("showReinvestCompound")} 
          user={user} 
          onSuccess={(updatedUser, amount) => updateUserAndSuccess(updatedUser, amount, "reinvest") } 
        />
      )} */}

      {successData && (
        <SuccessModal isOpen={!!successData} onClose={() => setSuccessData(null)} type={successData.type} userId={successData.userId} amount={successData.amount} />
      )}
    </>
  );
};
