...
// // import MessageModal from "./MessageModal";
// // import SuccessModal from "./SuccessModal";
// // import { ShieldCheck, CheckCircle, X, Eye, EyeOff, Zap } from "lucide-react"; 
// // import { useAuth } from "../../context/AuthContext";

// // const StakeCctModal = ({ isOpen, onClose, onSuccess, cctBalance, walletBalance, isReinvestMode }) => {
// //   const [userId, setUserId] = useState("");
// //   const [userInfo, setUserInfo] = useState(null);
// //   const [amount, setAmount] = useState("");
// //   const [transactionPassword, setTransactionPassword] = useState("");
...
// //     // 🔥 UPDATED VALIDATION: 10 to 2000 CCT limit
// //     if (!amount || amount < 10 || amount > 2000) return showMessage("Error", "❌ Compounding amount must be between 10 and 2000 CCT.", "error");
// //     if (!transactionPassword) return showMessage("Error", "❌ Enter transaction password.", "error");
    
// //     setLoading(true);
// //     try {
// //       // 🔥 Naya backend route call ho raha hai
// //       const endpoint = isPromo ? '/staking/cct-compound/promo' : '/staking/cct-compound';
// //       const payload = isPromo 
// //         ? { amount: Number(amount), transactionPassword } 
// //         : { targetUserId: userId, amount: Number(amount), transactionPassword };
...
// //   if (!isOpen) return null; // Added extra safety for rendering

// //   return (
// //     <>
// //       {!successOpen && (
// //         <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/70 backdrop-blur-sm p-4 overflow-hidden">
// //           <div className="bg-white w-full max-w-md flex flex-col rounded-3xl border border-slate-200 shadow-2xl overflow-hidden relative max-h-[75vh] animate-in zoom-in duration-300">
...
// //                 </div>
// //                 <div>
// //                   <h1 className="text-lg font-black text-slate-800 tracking-tight">CCT <span className="text-blue-600">Compounding</span></h1>
// //                 </div>
// //               </div>
// //               <button onClick={onClose} className="group bg-white hover:bg-red-50 p-1.5 rounded-full transition-all border border-slate-200 hover:border-red-200 shadow-sm cursor-pointer">
// //                  <X size={18} className="text-slate-400 group-hover:text-red-500" />
...
// //                  <div>
// //                     <span className="text-black text-[9px] uppercase tracking-wider font-bold block mb-0.5">Your CCT Balance</span>
// //                    <div className="text-sm md:text-base font-black text-blue-600 font-mono">
// //                       {isPromo ? "Demo Active" : `${(Math.floor(Number(cctBalance || 0) * 100) / 100).toFixed(2)} CCT`}
// //                    </div>
// //                  </div>
// //               </div>
...
// //                 {isPromo ? (
// //                   <div className="border border-blue-400 bg-blue-50 rounded-lg p-3 flex justify-between items-center transition-all shadow-sm">
// //                     <div>
// //                       <div className="text-blue-900 font-black text-[11px] md:text-xs">🚀 Promo Mode Active</div>
// //                       <div className="text-[10px] font-mono text-blue-700 mt-0.5">Target ID & Name will be auto-generated</div>
// //                     </div>
// //                   </div>
// //                 ) : (
// //                   <>
// //                     <div className="flex gap-2">
// //                       <div className="relative flex-1">
// //                         <input type="number" placeholder="Enter Target ID" value={userId} onChange={(e) => setUserId(e.target.value)} className={`w-full bg-slate-50 text-slate-800 rounded-lg px-3 py-2 outline-none transition-all placeholder-slate-400 font-mono shadow-inner text-xs md:text-sm ${userInfo ? 'border border-blue-500 ring-2 ring-blue-100' : 'border border-slate-200 focus:border-blue-400 focus:ring-2 focus:ring-blue-100'}`} />
// //                         {userInfo && <div className="absolute right-3 top-1/2 -translate-y-1/2 text-blue-500"><CheckCircle size={16} /></div>}
// //                       </div>
// //                       {!userInfo && (
// //                         <button onClick={() => fetchUser(userId, true)} className="bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-700 px-3 rounded-lg font-bold transition-all shadow-sm text-xs">Check</button>
// //                       )}
...
// //                     <div className="mt-1 min-h-[44px]">
// //                       {userInfo ? (
// //                         <div className="border border-blue-200 bg-blue-50 rounded-lg p-2 flex justify-between items-center transition-all shadow-sm">
// //                           <div>
// //                             <div className="text-blue-900 font-black text-[10px] md:text-xs">{userInfo.name}</div>
// //                             <div className="text-[9px] md:text-[10px] font-mono text-blue-700 mt-0.5">ID: {userInfo.userId}</div>
// //                           </div>
// //                         </div>
// //                       ) : (
// //                         userId.length > 0 && <div className="text-[9px] text-slate-400 italic px-2 pt-1">Checking ID details...</div>
...
// //               <div className="space-y-1 pt-1 border-t border-slate-100">
// //                 {/* 🔥 TEXT UPDATED TO REFLECT NEW LIMITS (10 - 2000) */}
// //                 <label className="text-[9px] md:text-[10px] font-bold text-black uppercase tracking-widest ml-1">Compound Amount (10 - 2000)</label>
// //                 <input type="number" placeholder="Enter CCT Amount" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full bg-slate-50 text-slate-800 rounded-lg px-3 py-2 outline-none transition-all placeholder-slate-400 font-mono shadow-inner text-xs md:text-sm border border-slate-200 focus:border-blue-400 focus:ring-2 focus:ring-blue-100" />
                
// //                 {/* 🔥 NEW RATE CHART TO SHOW 40 DAYS, 3% DAILY, 120% TOTAL */}
// //                 <div className="bg-blue-50/60 rounded-lg border border-blue-100 p-2.5 mt-3 shadow-inner">
// //                    <div className="text-center text-[10px] font-black text-blue-800 mb-2 tracking-widest">COMPOUNDING TERMS</div>
// //                    <div className="grid grid-cols-3 gap-2 text-center">
// //                      <div className="bg-white rounded-md p-1.5 border border-blue-100 shadow-sm">
// //                         <div className="text-[9px] font-bold text-slate-500 uppercase">Cycle</div>
// //                         <div className="text-[11px] font-black text-blue-600 mt-0.5">40 Days</div>
// //                      </div>
// //                      <div className="bg-white rounded-md p-1.5 border border-blue-100 shadow-sm">
// //                         <div className="text-[9px] font-bold text-slate-500 uppercase">Daily ROI</div>
// //                         <div className="text-[11px] font-black text-green-600 mt-0.5">3%</div>
// //                      </div>
// //                      <div className="bg-white rounded-md p-1.5 border border-blue-100 shadow-sm">
// //                         <div className="text-[9px] font-bold text-slate-500 uppercase">Total Return</div>
// //                         <div className="text-[11px] font-black text-amber-500 mt-0.5">120%</div>
// //                      </div>
// //                    </div>
                   
// //                    <p className="text-[9px] text-center text-slate-500 mt-2 font-medium px-2 leading-tight">
// //                      Note: Upline & User both need minimum <span className="font-bold text-slate-700">50 CCT Compounded</span> to release Direct/Level bonuses.
// //                    </p>
// //                 </div>
...
// //                       value={transactionPassword} 
// //                       onChange={(e) => setTransactionPassword(e.target.value)} 
// //                       className="w-full bg-white border border-slate-200 text-slate-800 rounded-lg p-2 pr-10 focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all placeholder-slate-400 font-mono shadow-sm text-xs md:text-sm" 
// //                     />
// //                     <button 
// //                       type="button"
// //                       onClick={() => setShowPassword(!showPassword)}
...
// //                     </button>
// //                  </div>
// //                  <button onClick={handleCompound} disabled={loading || (!isPromo && !userInfo)} className={`w-full py-2.5 rounded-lg font-black text-xs md:text-sm flex items-center justify-center gap-2 transition-all shadow-sm ${loading || (!isPromo && !userInfo) ? 'bg-slate-200 text-slate-500 cursor-not-allowed border border-slate-300' : 'bg-blue-600 hover:bg-blue-700 text-white shadow-[0_4px_15px_rgba(37,99,235,0.4)] hover:shadow-[0_6px_20px_rgba(37,99,235,0.6)] hover:-translate-y-0.5'}`}>
// //                    {loading ? "PROCESSING...." : "START COMPOUNDING"}
// //                  </button>
// //                  <button onClick={onClose} className="w-full py-2 rounded-lg font-bold text-xs bg-slate-100 hover:bg-slate-200 text-slate-600 transition-colors shadow-sm">Cancel</button>
// //               </div>
// //             </div>
...
// //         onClose={handleSuccessClose} 
// //         type="stake" 
// //         customTitle="COMPOUNDING"
// //         customMessage="has successfully started CCT Compounding"
// //         userId={successData.userId}
// //         userName={successData.userName} 
// //         amount={successData.amount} 
// //         zIndex={10000}
...
const StakeCctModal = ({ onClose, onSuccess, cctBalance, walletBalance }) => {
  const [userId, setUserId] = useState("");
  const [userInfo, setUserInfo] = useState(null);
  const [amount, setAmount] = useState("");
  const [cctAmountToUse, setCctAmountToUse] = useState(""); // 🔥 NAYA STATE: 50% CCT Rule ke liye
  const [transactionPassword, setTransactionPassword] = useState("");
  const [showPassword, setShowPassword] = useState(false);
  const [loading, setLoading] = useState(false);
  ...
  const handleStake = async () => {
    ...
    // Agar user CCT amount blank chhodta hai, toh system default 100% CCT maan lega
    const cctUse = cctAmountToUse !== "" ? Number(cctAmountToUse) : stakeAmt;

    // 🔥 UPDATED VALIDATION: 10 to 5000 CCT limit
    if (!amount || stakeAmt < 10 || stakeAmt > 5000) {
        return showMessage("Error", "❌ Staking amount must be between 10 and 5000.", "error");
    }
    // 🔥 VALIDATION: Min 50% CCT Rule
    if (cctUse < (stakeAmt * 0.5)) {
        return showMessage("Error", "❌ You must use at least 50% CCT balance for this transaction.", "error");
    }
    if (cctUse > stakeAmt) {
        return showMessage("Error", "❌ CCT usage cannot be greater than the total package amount.", "error");
    }
    if (!transactionPassword) return showMessage("Error", "❌ Enter transaction password.", "error");
    ...
    try {
      ...
      // 🔥 Payload me cctAmountToUse bhi backend bhej rahe hain
      const payload = isPromo 
        ? { amount: stakeAmt, cctAmountToUse: cctUse, transactionPassword } 
        : { targetUserId: userId, amount: stakeAmt, cctAmountToUse: cctUse, transactionPassword };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-hidden">
          <div className="bg-white w-full max-w-md flex flex-col rounded-3xl border border-slate-200 shadow-2xl overflow-hidden relative max-h-[75vh] animate-in zoom-in duration-300">
            ...
            <div className="flex-1 p-3 md:p-4 space-y-3 z-10 bg-white overflow-y-auto custom-scroll">
              ...
              {/* 🔥 DONO WALLET BALANCE DISPLAY (CCT & Wallet) */}
              <div className="flex justify-between items-center bg-white border border-slate-200 rounded-xl p-2 px-3 shadow-sm">
                 <div>
                    <span className="text-black text-[9px] uppercase tracking-wider font-bold block mb-0.5">Your CCT Balance</span>
                   <div className="text-sm md:text-base font-black text-green-600 font-mono">
                 ...
                 <div className="text-right border-l border-slate-100 pl-3">
                   ...
                   <div className="text-sm md:text-base font-black text-green-600 font-mono">
                      {isPromo ? "Demo Active" : `$${(Math.floor(Number(walletBalance || 0) * 100) / 100).toFixed(2)}`}
                   </div>
                 </div>
              </div>
              ...
              {/* 🔥 LIMIT UPDATED TO 10 - 5000 */}
              <div className="space-y-1 pt-1 border-t border-slate-100">
                <label className="text-[9px] md:text-[10px] font-bold text-black uppercase tracking-widest ml-1">Total Staking Amount (10 - 5000)</label>
                <input type="number" placeholder="Enter Total Amount" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full bg-slate-50 text-slate-800 rounded-lg px-3 py-2 outline-none transition-all placeholder-slate-400 font-mono shadow-inner text-xs md:text-sm border border-slate-200 focus:border-green-400 focus:ring-2 focus:ring-green-100" />
              </div>

              {/* 🔥 NAYA INPUT: 50% CCT DEDUCTION */}
              <div className="space-y-1 pt-1">
                <div className="flex justify-between">
                  {/* <label className="text-[9px] md:text-[10px] font-bold text-black uppercase tracking-widest ml-1">CCT To Use (Min 50%)</label> */}
                  {amount > 0 && (
                    <span className="text-[9px] font-bold text-green-600">
                      {/* Wallet: {amount - (cctAmountToUse !== "" ? Number(cctAmountToUse) : Number(amount))} */}
                    </span>
                  )}
                </div>
                {/* <input type="number" placeholder="Leave blank to use 100% CCT" value={cctAmountToUse} onChange={(e) => setCctAmountToUse(e.target.value)} className="w-full bg-slate-50 text-slate-800 rounded-lg px-3 py-2 outline-none transition-all placeholder-slate-400 font-mono shadow-inner text-xs md:text-sm border border-slate-200 focus:border-green-400 focus:ring-2 focus:ring-green-100" /> */}
              </div>

              {/* 🔥 TERMS UPDATED: 1% ROI aur 2x Cap */}
              <div className="bg-green-50/50 rounded-lg border border-green-100 p-2.5 mt-3 shadow-inner">
                 <div className="text-center text-[10px] font-black text-green-700 mb-2 tracking-widest">STAKING TERMS</div>
                 <div className="grid grid-cols-3 gap-2 text-center">
                   <div className="bg-white rounded-md p-1.5 border border-green-100 shadow-sm">
                      <div className="text-[9px] font-bold text-slate-500 uppercase">Max Cap</div>
                      <div className="text-[11px] font-black text-green-600 mt-0.5">2x Limit</div>
                   </div>
                   <div className="bg-white rounded-md p-1.5 border border-green-100 shadow-sm">
                      <div className="text-[9px] font-bold text-slate-500 uppercase">Daily ROI</div>
                      <div className="text-[11px] font-black text-blue-600 mt-0.5">1%</div>
                   </div>
                   <div className="bg-white rounded-md p-1.5 border border-green-100 shadow-sm">
                      <div className="text-[9px] font-bold text-slate-500 uppercase">Total Return</div>
                      <div className="text-[11px] font-black text-amber-500 mt-0.5">200%</div>
                   </div>
                 </div>
                 
                 <p className="text-[9px] text-center text-slate-500 mt-2 font-medium px-2 leading-tight">
                   Note: Minimum 50% of your transaction must be paid using your CCT Balance.
                 </p>
              </div>
