...
import MessageModal from "./MessageModal";
import SuccessModal from "./SuccessModal";
import { ShieldCheck, CheckCircle, X, Eye, EyeOff, Info } from "lucide-react"; 
import { useAuth } from "../../context/AuthContext";

// 🔥 NAYA PROPS: yahan walletBalance add kar diya hai
const StakeCctModal = ({ onClose, onSuccess, cctBalance, walletBalance }) => {
  ...
  return (
    <>
      {!successOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-hidden">
          <div className="bg-white w-full max-w-md flex flex-col rounded-3xl border border-slate-200 shadow-2xl overflow-hidden relative max-h-[80vh] animate-in zoom-in duration-300">
            ...
            <div className="flex-1 p-3 md:p-4 space-y-3 z-10 bg-white overflow-y-auto custom-scroll">
              ...
              {/* 🔥 NAYA FIX: DONO BALANCES YAHAN DIKHENGE GRID MEIN */}
              <div className="grid grid-cols-2 gap-2">
                 <div className="bg-white border border-slate-200 rounded-xl p-2 px-3 shadow-sm flex flex-col items-start">
                    <span className="text-slate-500 text-[9px] uppercase tracking-wider font-bold block mb-0.5">CCT Balance</span>
                    <div className="text-sm md:text-base font-black text-green-600 font-mono">
                      {isPromo ? "Demo" : `${(Math.floor(Number(cctBalance || 0) * 100) / 100).toFixed(2)}`}
                    </div>
                 </div>
                 <div className="bg-white border border-slate-200 rounded-xl p-2 px-3 shadow-sm flex flex-col items-start">
                    <span className="text-slate-500 text-[9px] uppercase tracking-wider font-bold block mb-0.5"> Wallet Balance</span>
                    <div className="text-sm md:text-base font-black text-blue-600 font-mono">
                      {isPromo ? "Demo" : `$${(Math.floor(Number(walletBalance || 0) * 100) / 100).toFixed(2)}`}
                    </div>
                 </div>
              </div>
              ...
              <div className="space-y-1 pt-1 border-t border-slate-100">
                ...
                <div className="bg-green-50/50 rounded-lg border border-green-100 p-2 mt-2">
                   <div className="text-center text-[9px] font-black text-green-700 mb-1">STAKING REWARDS (FIXED 3X CAP)</div>
                   <div className="grid grid-cols-4 gap-1 text-center">
                     <div className="bg-white rounded p-1 border border-green-100 shadow-sm">
                        <div className="text-[8px] font-bold text-slate-500">50-499</div>
                        <div className="text-[10px] font-black text-green-600">1% / Day</div>
                     </div>
                     <div className="bg-white rounded p-1 border border-green-100 shadow-sm">
