...
const FastTrackTimerBanner = ({ user }) => {
  ...
  const [showTimer, setShowTimer] = useState(false); // Ab iska naam showTimer kar diya hai

  useEffect(() => {
    // Agar topup date nahi hai, toh sirf timer hide hoga, banner nahi
    ...
    const updateTimer = () => {
      ...
      if (diff <= 0) {
        setShowTimer(false); // Timer khatam toh hide kar do
        return;
      }
      ...
      setShowTimer(true); // Timer chal raha hai
    };
