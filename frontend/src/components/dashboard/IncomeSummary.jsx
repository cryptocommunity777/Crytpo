...
const IncomeSummary = ({ income = {}, user = {} }) => {
  ...
  // GLOBAL GROWTH INCOME CALCULATION
  useEffect(() => {
    if (!user) return;
  ...
  // 🔥 UPDATE: Total Income box removed, keeping only 4 precise earning boxes
  const cardData = [
  ...
  return (
    <div className="w-full">
      {/* 🔥 UPDATE: 4 items ke liye grid-cols-2 ekdum perfect aur balanced rahega */}
      <div className="grid grid-cols-2 gap-3 md:gap-4">
        {cardData.map((item, index) => (
          <div key={index} className="relative overflow-hidden p-4 rounded-xl border border-slate-200 bg-white hover:shadow-md transition-shadow duration-300 group shadow-sm">
            ...
            <div className={`absolute -right-4 -top-4 w-20 h-20 rounded-full ${item.glow} blur-[20px] group-hover:blur-[30px] transition-all duration-500 opacity-50`}></div>
