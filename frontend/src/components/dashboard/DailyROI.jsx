...
const globalPoolConfig = {
  totalTarget: 12200,
  levels: [
    { level: 1,  globalTeam: 20,    requiredLevelTeam: 1,   daily: 1,  days: 10,  earning: 10   }, 
    { level: 2,  globalTeam: 40,    requiredLevelTeam: 2,   daily: 1,  days: 20,  earning: 20   }, 
    { level: 3,  globalTeam: 100,   requiredLevelTeam: 4,   daily: 1,  days: 40,  earning: 40   }, 
    { level: 4,  globalTeam: 200,   requiredLevelTeam: 8,   daily: 1,  days: 80,  earning: 80   }, 
    { level: 5,  globalTeam: 400,   requiredLevelTeam: 15,  daily: 1,  days: 150, earning: 150  }, 
    { level: 6,  globalTeam: 1600,  requiredLevelTeam: 20,  daily: 1,  days: 200, earning: 200  }, 
    { level: 7,  globalTeam: 2000,  requiredLevelTeam: 50,  daily: 2,  days: 250, earning: 500  }, 
    { level: 8,  globalTeam: 3000,  requiredLevelTeam: 70,  daily: 2,  days: 350, earning: 700  }, 
    { level: 9,  globalTeam: 4000,  requiredLevelTeam: 100, daily: 2,  days: 500, earning: 1000 }, 
    { level: 10, globalTeam: 5000,  requiredLevelTeam: 150, daily: 3,  days: 500, earning: 1500 }, 
    { level: 11, globalTeam: 7500,  requiredLevelTeam: 300, daily: 6,  days: 500, earning: 3000 }, 
    { level: 12, globalTeam: 10000, requiredLevelTeam: 500, daily: 10, days: 500, earning: 5000 }  
  ]
};
...
export default function Plan() {
  ...
  // 🔥 YAHAN SE DATA AAYEGA BACKEND SE
  const userGlobalTeam = user?.globalTeamCount || 0; 
  const userDirects = user?.directCount || 0;
  const isToppedUp = user?.isToppedUp || false;

  // 🛡️ API SE JO OBJECT AAYA { 1: 5, 2: 10 }, USKO STABLE BANANE KE LIYE STRINGIFY
  const backendLevelData = JSON.stringify(user?.levelActiveCounts || {});

  // ---------------- PROCESS DATA ----------------
  useEffect(() => {
    let cumulativeGlobalTeam = 0; 
    
    // 🛡️ STRING KO WAPAS OBJECT MEIN CONVERT KIYA
    const levelCounts = JSON.parse(backendLevelData);

    const processedLevels = globalPoolConfig.levels.map((staticLvl) => {
      const previousCumulative = cumulativeGlobalTeam; 
      cumulativeGlobalTeam += staticLvl.globalTeam; 
      
      // 🔥 EXACT MATCH: Backend se specific level ka count uthao (Fallback 0)
      // Example: Agar staticLvl.level '1' hai, toh levelCounts['1'] ka data nikalega
      const currentLevelUsers = levelCounts[staticLvl.level] || levelCounts[staticLvl.level.toString()] || 0;
      
      let status = "TEAM_PENDING"; 
      const isTeamReady = userGlobalTeam >= cumulativeGlobalTeam;
      const isLevelReady = currentLevelUsers >= staticLvl.requiredLevelTeam;

      if (isTeamReady && isLevelReady) {
        status = "ACHIEVED";
      } else if (isTeamReady && !isLevelReady) {
      ...
      return { 
        ...
        currentLevelProgress, 
        cumulativeGlobalTeam,
        currentLevelUsers // 🔥 YEH VALUE TABLE MEIN (Current: X) DIKHAYEGI
      };
    });

    setActiveData({ ...globalPoolConfig, levels: processedLevels });
  }, [userGlobalTeam, backendLevelData]); 
                    ...
                    <table className="min-w-full text-sm md:text-base">
                      <thead className="bg-slate-100">
                        <tr>
                          ...
                          <th className="px-4 py-3 text-center font-bold text-slate-700">Withdrawal</th>
                          <th className="px-4 py-3 text-center font-bold text-slate-700">Wallet</th>
                          <th className="px-4 py-3 text-center font-bold text-slate-700">Community Net Withdrawal</th>
                          <th className="px-4 py-3 text-center font-bold text-slate-700">Community Net Wallet</th>
                        </tr>
                      </thead>
                ...
                <table className="w-full text-[10px] md:text-sm text-left whitespace-nowrap">
                  ...
                  <thead className="text-slate-600 text-[9px] md:text-xs uppercase tracking-widest border-b border-slate-200 bg-slate-50">
                    <tr>
                      <th className="py-3 px-2 md:py-4 md:px-4 font-black text-center">Lvl</th>
                      <th className="py-3 px-2 md:py-4 md:px-4 font-black text-center">Community Team</th>
                      <th className="py-3 px-2 md:py-4 md:px-4 font-black text-center">Level Team</th>
                      <th className="py-3 px-2 md:py-4 md:px-4 font-black text-center">Total Earning</th>
                      <th className="py-3 px-2 md:py-4 md:px-4 font-black text-center">Status</th>
                    </tr>
                  </thead>
                  ...
                  <tbody>
                    {activeData.levels.map((lvl) => {
                      const isAchieved = lvl.status === "ACHIEVED";
                      const isPoolFilled = lvl.status === "ACHIEVED" || lvl.status === "LEVEL_PENDING";
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* 🔥 LEVEL TEAM COLUMN (YAHAN PE CURRENT VALUE RENDER HOGI) */}
                          <td className="py-2.5 px-2 md:py-3 md:px-4 text-center">
                            <div className="flex flex-col items-center justify-center gap-1">
                               <span className={`px-2.5 py-1 md:px-3 md:py-1.5 rounded-md border text-[9px] md:text-xs font-black inline-block ${
                                 lvl.currentLevelUsers >= lvl.requiredLevelTeam 
                                 ? 'bg-emerald-100/50 border-emerald-200 text-emerald-700' 
                                 : 'bg-amber-50 border-amber-200 text-amber-700'
                               }`}>
                                 Target: {lvl.requiredLevelTeam}
                               </span>
                               <div className="text-[8px] md:text-[9px] font-bold tracking-wider text-slate-500">
                                 (Current: {lvl.currentLevelUsers})
                               </div>
                            </div>
                          </td>
                          ...
                          <td className="py-2.5 px-2 md:py-3 md:px-4 text-center">
                            ...
                            {lvl.status === "LEVEL_PENDING" && (
                              <div className="inline-flex items-center justify-center gap-1 text-amber-700 bg-amber-100 px-2.5 py-1 rounded border border-amber-200 font-black text-[9px] md:text-[11px] uppercase tracking-wide">
                                <UserPlus size={12} strokeWidth={3} /> Team Req
                              </div>
                            )}
                            {lvl.status === "TEAM_PENDING" && (
                              <div className="inline-flex items-center justify-center gap-1 text-slate-500 bg-slate-100 px-2.5 py-1 rounded border border-slate-200 font-black text-[9px] md:text-[11px] uppercase tracking-wide">
