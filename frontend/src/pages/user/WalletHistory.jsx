...
const WalletHistory = () => {
  ...
  // 🔥 ROLE & LIVE BALANCES STATE
  const [userRole, setUserRole] = useState("");
  const [liveBalances, setLiveBalances] = useState({ main: 0, usdt: 0, cct: 0 }); 
  ...
  // 🌟 USDT TRANSFER ADDED
  const allTypes = [
    "deposit", "credit_to_wallet", "credit", "transfer", "usdt_bep20_transfer", "topup", 
    "debit_topup", "withdrawal", "manual_credit", "manual_debit", "fast_track",
    "convert_to_cct", "cct_stake_send", "cct_transfer"
  ];

  const dropdownTypes = [
    "all", "deposit", "credit_to_wallet", "credit", "transfer", "usdt_bep20_transfer",
    "topup", "withdrawal", "fast_track", "convert_to_cct", "cct_stake_send", "cct_transfer"
  ];
  ...
  const fetchWalletHistory = async (uid) => {
    try {
      ...
      // 🌟 YAHAN DATABASE SE DIRECT BALANCE SET HO RAHA HAI
      if (res.data && res.data.balances) {
          setLiveBalances({
              main: res.data.balances.walletBalance || 0,
              usdt: res.data.balances.usdtBep20Balance || 0,
              cct: res.data.balances.cctBalance || 0 
          });
      }
      ...
      const formattedHistory = txns
        ...
        .filter(t => {
          ...
          // 🔥 DUPLICATE P2P ENTRY FIX (MAIN, USDT & CCT)
          if (t.type === 'transfer' || t.type === 'usdt_bep20_transfer' || t.type === 'cct_transfer' || t.type === 'credit') {
              const isSender = String(t.fromUserId) === String(uid) || String(t.userId) === String(uid);
              const isReceiver = String(t.toUserId) === String(uid);
              
              if (!isSender && !isReceiver) return false;
              
              // Agar user sirf SENDER hai
              if (isSender && !isReceiver) {
                  if (desc.includes("received")) return false; // Receiver ki entry hide karo
                  // Agar backend USDT Transfer ka receiver 'credit' banata hai, toh sender ko wo nahi dikhna chahiye
                  if (t.type === 'credit' && (t.source === 'usdt_bep20_transfer' || desc.includes("usdt bep20"))) return false;
              }

              // Agar user sirf RECEIVER hai
              if (isReceiver && !isSender) {
                  if (desc.includes("transferred")) return false; // Sender ki entry hide karo
              }
          }
  ...
  const calculateBalances = () => {
    ...
    let usdtRunningBalance = 0; 
    let cctRunningBalance = 0; 

    const july13Deadline = new Date("2026-07-13T16:00:00+05:30"); 
    const june30 = new Date("2026-06-30T00:00:00+05:30");

    return transactions.map((txn) => {
      let mathImpact = 0;
      let usdtMathImpact = 0; 
      let cctMathImpact = 0; 
      let colorStyle = "text-black";
      let operator = "";
      let finalDescription = txn.description || "";
      ...
      switch (txn.type) {
        case "deposit":
          if (txnDate >= july13Deadline) {
              usdtMathImpact = amt;
              colorStyle = "text-cyan-600";
              displayTypeUI = "DEPOSIT (USDT)";
              icon = <ArrowDownLeft size={14} className="text-cyan-600" />;
          } else {
              mathImpact = amt;
              colorStyle = "text-green-500";
              displayTypeUI = "DEPOSIT";
              icon = <ArrowDownLeft size={14} className="text-green-500" />;
          }
          operator = "+";
          break;
        ...
        case "credit":
          // 🔥 USDT aur Main Credit alag kiya
          if (txn.source === 'usdt_bep20_transfer' || descLower.includes('usdt bep20')) {
              usdtMathImpact = amt;
              colorStyle = "text-cyan-600";
              displayTypeUI = "USDT RECEIVED";
              icon = <ArrowDownLeft size={14} className="text-cyan-600" />;
          } else {
              mathImpact = amt;
              colorStyle = "text-green-500";
        ...
        case "fast_track":
          if (txnDate >= july13Deadline) {
              usdtMathImpact = amt;
              colorStyle = "text-cyan-600";
              displayTypeUI = "FAST TRACK (USDT)";
              icon = <Zap size={14} className="text-cyan-600" />;
          } else {
              mathImpact = amt;
              colorStyle = "text-blue-500"; 
              displayTypeUI = "FAST TRACK";
        ...
        case "usdt_bep20_transfer":
          if (toId === myId) {
            ...
            displayTypeUI = "USDT RECEIVED";
            icon = <ArrowDownLeft size={14} className="text-cyan-600" />;
            finalDescription = `Received $${amt} USDT from ID #${fromId || txnOwnerId}`;
          } else if (fromId === myId || (!fromId && txnOwnerId === myId)) {
            usdtMathImpact = -amt;
            colorStyle = "text-red-500";
            operator = "-";
            displayTypeUI = "USDT SENT";
            icon = <ArrowRightLeft size={14} className="text-red-500" />;
          }
          break;
        ...
        case "transfer":
          // 🔥 USDT Transfer vs Main Wallet Transfer alag kiya
          const isUsdtTransfer = txn.source === 'usdt_bep20' || descLower.includes('usdt bep20');
          if (toId === myId) {
            if (isUsdtTransfer) {
                usdtMathImpact = amt;
                colorStyle = "text-cyan-600";
        ...
        case "debit_topup":
          ...
          if (isMyMoneySpent) {
            if (amt === 10 && finalDescription.includes("Pre-launch")) {
              ...
              if (txn.source === 'usdt_bep20' || descLower.includes('usdt bep20')) {
                  usdtMathImpact = -amt;
                  colorStyle = "text-red-500";
                  operator = "-";
                  displayTypeUI = "TOPUP (USDT)";
              } else {
                  mathImpact = -amt; 
                  colorStyle = "text-red-500";
                  operator = "-";
                  displayTypeUI = "TOPUP (MAIN)";
              }
            }
          } else {
            mathImpact = 0;
      ...
      if (userRole !== "leader") {
          runningBalance += mathImpact;
          usdtRunningBalance += usdtMathImpact; 
          cctRunningBalance += cctMathImpact;
      }
      ...
      return {
        ...
        description: finalDescription,
        balance: userRole === "leader" ? "0.00" : (runningBalance || 0).toFixed(2),
        usdtBalance: userRole === "leader" ? "0.00" : (usdtRunningBalance || 0).toFixed(2),
        cctBalance: userRole === "leader" ? "0.00" : (cctRunningBalance || 0).toFixed(2),
        colorStyle,
        formattedAmount: finalFormattedAmt,
        displayTypeUI,
  ...
  const currentWalletBalance = userRole === "leader" ? "0.00" : Number(liveBalances.main).toFixed(2);
  const currentUSDTBalance = userRole === "leader" ? "0.00" : Number(liveBalances.usdt).toFixed(2);
  const currentCCTBalance = userRole === "leader" ? "0.00" : (liveBalances.cct > 0 ? Number(liveBalances.cct).toFixed(2) : (processedData.length > 0 ? processedData[processedData.length - 1].cctBalance : "0.00"));
  ...
  return (
    <div className="w-full max-w-7xl mx-auto pb-10 relative z-10 animate-in fade-in duration-500">
      ...
      {/* 🔥 TRIPLE BALANCE CARDS (Main, USDT, CCT) */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-white shadow-sm backdrop-blur-md rounded-2xl border border-slate-200 p-5 md:p-6 shadow-[0_0_30px_rgba(34,197,94,0.1)] relative overflow-hidden flex flex-col justify-center">
          <div className="absolute top-0 right-0 w-24 h-24 bg-green-500/10 blur-[30px]"></div>
          <h3 className="text-gray-500 text-[10px] md:text-xs font-black uppercase tracking-widest flex items-center gap-2 mb-2">
             <Wallet size={14} className="text-green-500" /> Current Main Balance
          </h3>
        ...
        <div className="bg-white shadow-sm backdrop-blur-md rounded-2xl border border-slate-200 p-5 md:p-6 shadow-[0_0_30px_rgba(6,182,212,0.1)] relative overflow-hidden flex flex-col justify-center">
          <div className="absolute top-0 right-0 w-24 h-24 bg-cyan-500/10 blur-[30px]"></div>
          <h3 className="text-gray-500 text-[10px] md:text-xs font-black uppercase tracking-widest flex items-center gap-2 mb-2">
             <Wallet size={14} className="text-cyan-500" /> USDT BEP20 Balance
          </h3>
          <p className="text-3xl md:text-4xl font-black text-slate-900 drop-shadow-md">
            ${currentUSDTBalance}
          </p>
          ...
          <table className="w-full text-xs sm:text-sm text-left whitespace-nowrap">
            <thead className="bg-slate-50 text-green-600 text-[10px] md:text-xs uppercase tracking-widest border-b border-slate-200">
              <tr>
                ...
                <th className="p-4 font-black">Type</th>
                <th className="p-4 font-black">Amount</th>
                <th className="p-4 font-black text-green-600">Main Bal.</th>
                <th className="p-4 font-black text-cyan-600">USDT Bal.</th>
                <th className="p-4 font-black text-indigo-600">CCT Bal.</th>
                <th className="p-4 font-black">From / To</th>
                <th className="p-4 font-black">Details</th>
              </tr>
                ...
                currentItems.map((txn, idx) => {
                  ...
                  return (
                    <tr key={`${txn._id}-${txn.date}-${idx}`} className="border-b border-slate-100 hover:bg-slate-50 transition-colors bg-white">
                      ...
                      <td className="p-4 font-black text-cyan-600">
                         ${txn.usdtBalance}
                      </td>
                      <td className="p-4 font-black text-indigo-600">
                         {txn.cctBalance}
