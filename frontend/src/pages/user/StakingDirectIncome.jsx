...
import { Search, ChevronLeft, ChevronRight, UserCircle, Coins } from "lucide-react";

const StakingDirectIncome = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    // 🔥 Backend se sirf Staking Direct Income fetch karega
    api.get(`/transaction/transactions/${userId}?type=staking_direct_income&t=${new Date().getTime()}`)
      .then((res) => {
      ...
      .catch((err) => {
        console.error("Failed to fetch CCT Staking Direct Income", err);
        setTransactions([]);
        setFiltered([]);
      })
      .finally(() => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto pb-10 relative z-10 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
        <div>
          <h2 className="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-purple-600 uppercase tracking-wide flex items-center gap-3">
             <Coins className="text-indigo-500" size={28} /> CCT Staking Direct
          </h2>
          <p className="text-black text-xs md:text-sm font-bold tracking-widest uppercase mt-1">
            Track your 10% direct staking rewards
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6 justify-between items-center bg-white shadow-sm p-4 rounded-2xl border border-slate-200">
        <div className="relative w-full sm:w-80 group">
           ...
           <input
             ...
             className="w-full bg-white border border-slate-200 text-slate-900 text-sm font-bold tracking-wide rounded-xl px-4 py-3 pl-10 focus:border-indigo-500 focus:outline-none transition-all placeholder-slate-400"
           />
        </div>
      </div>
      ...
      <div className="bg-white shadow-sm backdrop-blur-xl rounded-2xl border border-slate-200 overflow-hidden shadow-2xl relative">
        ...
        <div className="overflow-x-auto custom-scroll w-full relative z-10">
          <table className="w-full text-xs sm:text-sm text-left whitespace-nowrap">
            <thead className="bg-slate-50 text-indigo-600 text-[10px] md:text-xs uppercase tracking-widest border-b border-slate-200">
              <tr>
                  ...
                  return (
                    <tr key={txn._id || idx} className="border-b border-slate-100 hover:bg-white/5 transition-colors bg-white">
                      ...
                      <td className="p-4 font-black text-center">
                        <span className="text-indigo-600 drop-shadow-sm text-base">
        ...
        {!loading && filtered.length > 0 && (
           <div className="p-4 border-t border-slate-100 bg-slate-50/40 flex flex-col sm:flex-row justify-between items-center gap-4 relative z-10">
              ...
              <div className="flex items-center gap-2">
                 <button onClick={handlePrev} disabled={currentPage === 1} className={`p-2 rounded-lg flex items-center justify-center transition-all ${currentPage === 1 ? 'bg-white/5 text-gray-600 cursor-not-allowed' : 'bg-white text-slate-900 border hover:bg-indigo-50 hover:text-indigo-600'}`}>
                   <ChevronLeft size={18} />
                 </button>
                 <span className="bg-white border border-slate-200 text-slate-900 text-xs font-bold px-4 py-2 rounded-lg">
                    {currentPage} / {totalPages}
                 </span>
                 <button onClick={handleNext} disabled={currentPage === totalPages} className={`p-2 rounded-lg flex items-center justify-center transition-all ${currentPage === totalPages ? 'bg-white/5 text-gray-600 cursor-not-allowed' : 'bg-white text-slate-900 border hover:bg-indigo-50 hover:text-indigo-600'}`}>
...
export default StakingDirectIncome;