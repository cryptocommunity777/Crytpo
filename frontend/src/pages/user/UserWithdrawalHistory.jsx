...
//     // 🔥 TIME BOUNDARY: 12 July 2026, 4:30 PM IST
//     const BUG_FIX_TIMESTAMP = new Date("2026-07-12T16:30:00+05:30").getTime();

//     for (const item of sorted) {
//       const itemTime = new Date(item.createdAt).getTime();
//       const lastGroup = groups[groups.length - 1];

//       // 🔥 LOGIC: Agar transaction 4:30 PM se pehle ka hai to 2 se multiply karo, warna 1 (normal) rakho.
//       const multiplier = itemTime < BUG_FIX_TIMESTAMP ? 2 : 1;
...
//         lastGroup.rawGross += Number(item.grossAmount || 0) * multiplier;
//         lastGroup.rawFee += Number(item.fee || 0) * multiplier;
//         lastGroup.rawCryptoNet += Number(item.netAmount || 0);
//         lastGroup.rawTopupNet += Number(item.netAmount || 0);
...
//           createdAt: item.createdAt,
//           entries: [item],
//           rawGross: Number(item.grossAmount || 0) * multiplier,
//           rawFee: Number(item.fee || 0) * multiplier,
//           rawCryptoNet: Number(item.netAmount || 0),
//           rawTopupNet: Number(item.netAmount || 0),
//           walletAddress: item.walletAddress,
//           status: item.status || "pending",
