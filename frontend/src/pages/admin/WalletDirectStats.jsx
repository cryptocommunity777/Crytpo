...
import { saveAs } from 'file-saver';
import Papa from 'papaparse';
import { Copy, ArrowUpDown, ChevronUp, ChevronDown, Calendar, Crown, User, Info } from 'lucide-react';
...
const WalletDirectStats = () => {
  ...
  // 🔥 NAYA TOTALS CALCULATION (Added Net USDT Paid)
  // const totals = useMemo(() => {
  //   return processedUsers.reduce((acc, user) => {
  //     acc.marketWallet += (Number(user.walletBalance) || 0);
  //     acc.usdtBep20 += (Number(user.usdtBep20Balance) || 0); 
  //     acc.totalIn += (Number(user.totalIn) || 0);
  //     acc.fastTrack += (Number(user.fastTrackIncome) || 0);
  //     acc.p2p += (Number(user.p2pReceived) || 0);
  //     acc.deposit += (Number(user.totalDeposit) || 0);
  //     acc.creditToWallet += (Number(user.creditToWallet) || 0); 
  //     acc.leaderBonus += (Number(user.leaderBonus) || 0); 
  //     acc.withdraw += (Number(user.totalWithdrawal) || 0);
  //     acc.netPaid += (Number(user.netUsdtPaid) || 0); // 🌟 Added
  //     return acc;
  //   }, { marketWallet: 0, usdtBep20: 0, totalIn: 0, fastTrack: 0, p2p: 0, deposit: 0, creditToWallet: 0, leaderBonus: 0, withdraw: 0, netPaid: 0 });
  // }, [processedUsers]);
  ...
  const totals = useMemo(() => {
    return processedUsers.reduce((acc, user) => {
      ...
      // Withdrawal Logic
      acc.withdraw += (Number(user.totalWithdrawal) || 0);
      acc.netPaid += (Number(user.netUsdtPaid) || 0);
  ...
  const exportToCSV = () => {
    const csvData = processedUsers.map((u, i) => ({
      ...
      Role: u.role === 'leader' ? 'Leader' : 'Normal',
      PaidDirects: u.paidDirectCount || 0,
      TopupWalletBalance: (u.walletBalance || 0).toFixed(2),
      USDT_BEP20_Balance: (u.usdtBep20Balance || 0).toFixed(2),
      TotalIn_Filtered: (u.totalIn || 0).toFixed(2),
      P2PReceived: (u.p2pReceived || 0).toFixed(2),
      Deposits: (u.totalDeposit || 0).toFixed(2),
      FastTrackIncome: (u.fastTrackIncome || 0).toFixed(2),
      LeaderBonus: (u.leaderBonus || 0).toFixed(2), 
      CreditToWallet: (u.creditToWallet || 0).toFixed(2),
      Withdrawals_Gross: (u.totalWithdrawal || 0).toFixed(2),
      NetPaid_USDT: (u.netUsdtPaid || 0).toFixed(2) // 🌟 Added
    }));
    const csv = Papa.unparse(csvData);
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    saveAs(blob, `wallet-breakdown-report-${getISTDateStr(new Date())}.csv`);
  ...
  const SortableHeader = ({ label, sortKey, align = "text-center", width = "auto" }) => {
    const isActive = sortConfig.key === sortKey;
    return (
      <th 
        className={`px-2 py-3 border-b bg-slate-100 hover:bg-slate-200 cursor-pointer transition-colors select-none group whitespace-nowrap ${align}`}
        style={{ width: width }}
        onClick={() => handleSort(sortKey)}
      >
        <div className={`flex items-center gap-1.5 ${align === 'text-center' ? 'justify-center' : 'justify-between'}`}>
  ...
  return (
    <div className="p-4 pt-12 md:pt-16 max-w-[1600px] mx-auto w-full">
      ...
      {/* 🔥 ADMIN SYSTEM GUIDE BOX 🔥 */}
      <div className="mb-6 bg-blue-50 border border-blue-200 rounded-xl p-4 sm:p-5 shadow-sm">
        <h3 className="text-blue-800 font-black flex items-center gap-2 mb-3 text-sm md:text-base uppercase tracking-widest">
          <Info size={18} /> Admin System Guide & Logic
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-blue-900">
          <div className="bg-white p-3 rounded-lg border border-blue-100 shadow-sm">
            <strong className="block text-indigo-700 mb-1">🪙 USDT BEP20 Wallet:</strong>
            <ul className="list-disc pl-4 space-y-1">
              <li>Receives <b>Deposits</b> (After July 13, 4PM).</li>
              <li>Receives <b>FastTrack Income</b> (After July 14).</li>
              <li>Receives <b>Leader 10% Instant Bonus</b>.</li>
            </ul>
          </div>
          <div className="bg-white p-3 rounded-lg border border-blue-100 shadow-sm">
            <strong className="block text-green-700 mb-1">🏦 Withdrawal Math:</strong>
            <ul className="list-disc pl-4 space-y-1">
              <li><b>Gross:</b> The total requested by user.</li>
              <li><b>Net Paid:</b> Actual amount approved/sent to user's USDT BEP20 address.</li>
            </ul>
          </div>
          <div className="bg-white p-3 rounded-lg border border-blue-100 shadow-sm">
            <strong className="block text-amber-700 mb-1">🔄 Credit to Wallet (C2W):</strong>
            <ul className="list-disc pl-4 space-y-1">
              <li>Shows the amount <b>Returned to Topup Wallet</b> based on community split rule.</li>
              <li>Added directly to the user's "Current Topup Wallet".</li>
            </ul>
          </div>
        </div>
      </div>

      {/* 🔥 SUMMARY CARDS (Now 10 Cards) 🔥 */}
     {/* 🔥 SUMMARY CARDS: Responsive Grid (Mobile: 2, Tablet: 3, Desktop: 5) */}
<div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-6">
    <SummaryCard label="Topup Wallet" value={`$${totals.marketWallet.toFixed(2)}`} color="bg-indigo-50 border-indigo-200 text-indigo-700" />
    <SummaryCard label="USDT BEP20" value={`$${totals.usdtBep20.toFixed(2)}`} color="bg-amber-100 border-amber-300 text-amber-800" />
    <SummaryCard label="Total In (Filtered)" value={`$${totals.totalIn.toFixed(2)}`} color="bg-slate-50 border-slate-200 text-slate-700" />
    <SummaryCard label="Gross Withdrawals" value={`$${totals.withdraw.toFixed(2)}`} color="bg-red-50 border-red-200 text-red-700" />
    <SummaryCard label="Net Paid (USDT)" value={`$${totals.netPaid.toFixed(2)}`} color="bg-emerald-100 border-emerald-300 text-emerald-800" />

    <SummaryCard label="Deposits" value={`$${totals.deposit.toFixed(2)}`} color="bg-blue-50 border-blue-200 text-blue-700" />
    <SummaryCard label="Leader 10% Bonus" value={`$${totals.leaderBonus.toFixed(2)}`} color="bg-rose-50 border-rose-200 text-rose-700" />
    <SummaryCard label="FastTrack" value={`$${totals.fastTrack.toFixed(2)}`} color="bg-green-50 border-green-200 text-green-700" />
    <SummaryCard label="P2P Received" value={`$${totals.p2p.toFixed(2)}`} color="bg-teal-50 border-teal-200 text-teal-700" />
    <SummaryCard label="Credit 2 Wallet (C2W)" value={`$${totals.creditToWallet.toFixed(2)}`} color="bg-purple-50 border-purple-200 text-purple-700" />
</div>
      ...
      <div className="border border-gray-200 rounded-xl shadow-sm bg-white overflow-hidden">
        <div className="overflow-x-auto custom-scroll w-full">
          <table className="w-full text-xs lg:text-sm text-left">
            <thead className="text-gray-700 uppercase text-[9px] font-black tracking-wider">
              <tr>
                <th className="px-2 py-3 border-b bg-slate-100 text-center w-8 whitespace-nowrap">Sr.</th>
                <SortableHeader label="User ID" sortKey="userId" align="text-center" />
                <SortableHeader label="User Name" sortKey="name" align="text-left" />
                <SortableHeader label="Role" sortKey="role" align="text-center" />
                
                <SortableHeader label="Topup Wal" sortKey="walletBalance" align="text-center" />
                <SortableHeader label="USDT BEP20" sortKey="usdtBep20Balance" align="text-center" />
                
                <SortableHeader label={isToday ? "Today In" : isFiltered ? "Filter In" : "Total In"} sortKey="totalIn" align="text-center" />
                <SortableHeader label={isToday ? "Today P2P" : "P2P In"} sortKey="p2pReceived" align="text-center" />
                <SortableHeader label={isToday ? "Today Dep" : "Deposits"} sortKey="totalDeposit" align="text-center" />
                <SortableHeader label={isToday ? "Tdy L.Bonus" : "L. Bonus"} sortKey="leaderBonus" align="text-center" />
                <SortableHeader label={isToday ? "Today Fast" : "FastTrack"} sortKey="fastTrackIncome" align="text-center" />
                <SortableHeader label={isToday ? "Today C2W" : "C2W"} sortKey="creditToWallet" align="text-center" />
                <SortableHeader label={isToday ? "Gross W/D" : "Gross W/D"} sortKey="totalWithdrawal" align="text-center" />
                {/* 🌟 NEW NET PAID HEADER */}
                <SortableHeader label={isToday ? "Net Paid" : "Net Paid"} sortKey="netUsdtPaid" align="text-center" />
              </tr>
            </thead>
            
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                <tr><td colSpan="14" className="text-center py-10 text-indigo-600 font-black tracking-widest uppercase animate-pulse">⏳ Fetching & Calculating Data...</td></tr>
              ) : currentItems.length === 0 ? (
                <tr><td colSpan="14" className="text-center py-10 font-bold text-gray-500 bg-gray-50 uppercase tracking-widest">No matching users found.</td></tr>
              ) : (
                currentItems.map((user, idx) => {
                  const isLeader = user.role === 'leader';
                  return (
                    <tr 
                      ...
                      className={`transition-colors ${isLeader ? 'bg-red-50 hover:bg-red-100 border-b-2 border-red-200' : 'bg-white hover:bg-indigo-50/50'}`}
                    >
                      <td className={`px-2 py-3 text-center font-bold whitespace-nowrap ${isLeader ? 'text-red-600' : 'text-gray-500'}`}>
                        {indexOfFirstItem + idx + 1}
                      </td>
                      <td className="px-2 py-3 text-center whitespace-nowrap">
                        <div className="flex items-center justify-center gap-1">
                           <span className={`font-black ${isLeader ? 'text-red-700' : 'text-gray-800'}`}>#{user.userId}</span>
                           <div className={`flex items-center gap-1 border rounded p-0.5 shadow-sm ${isLeader ? 'bg-red-100 border-red-200' : 'bg-white border-gray-200'}`}>
                             <button onClick={() => handleCopy(user.userId.toString())} className={`transition p-1 ${isLeader ? 'text-red-500 hover:text-red-700' : 'text-gray-400 hover:text-gray-700'}`} title="Copy ID"><Copy size={10}/></button>
                           </div>
                        </div>
                      </td>
                      <td className="px-2 py-3 font-semibold capitalize truncate max-w-[100px]" title={user.name}>
                        <div className="flex items-center gap-2">
                          <span className={isLeader ? 'text-red-800 font-black' : 'text-gray-800'}>{user.name || "Unknown"}</span>
                        </div>
                      </td>
                      <td className="px-2 py-3 text-center whitespace-nowrap">
                         {isLeader ? (
                           <span className="inline-flex items-center gap-1 bg-red-100 text-red-800 border border-red-300 px-1 py-0.5 rounded text-[8px] font-black uppercase tracking-widest">
                             <Crown size={9} /> Leader
                           </span>
                         ) : (
                           <span className="inline-flex items-center gap-1 bg-blue-100 text-blue-800 border border-blue-200 px-1 py-0.5 rounded text-[8px] font-black uppercase tracking-widest">
                             <User size={9} /> Normal
                           </span>
                         )}
                      </td>
                      ...
                      <td className="px-2 py-3 text-center whitespace-nowrap">
                         <span className="text-amber-700 bg-amber-50 px-1.5 py-0.5 rounded border border-amber-200 font-black shadow-sm">
                           ${(user.usdtBep20Balance || 0).toFixed(2)}
                         </span>
                      </td>
                      <td className="px-2 py-3 text-center whitespace-nowrap">
                      ...
                      <td className="px-2 py-3 text-center whitespace-nowrap font-bold text-teal-600">
                         ${(user.p2pReceived || 0).toFixed(2)}
                      </td>
                      <td className="px-2 py-3 text-center whitespace-nowrap font-bold text-blue-600">
                         ${(user.totalDeposit || 0).toFixed(2)}
                      </td>
                      <td className="px-2 py-3 text-center whitespace-nowrap font-black text-rose-600 bg-rose-50/50">
                         ${(user.leaderBonus || 0).toFixed(2)}
                      </td>
                      <td className="px-2 py-3 text-center whitespace-nowrap font-bold text-green-600">
                         ${(user.fastTrackIncome || 0).toFixed(2)}
                      </td>
                      <td className="px-2 py-3 text-center whitespace-nowrap font-bold text-purple-600">
                         ${(user.creditToWallet || 0).toFixed(2)}
                      </td>
                      <td className="px-2 py-3 text-center whitespace-nowrap font-bold text-red-500">
                         ${(user.totalWithdrawal || 0).toFixed(2)}
                      </td>
                      
                      {/* 🌟 NEW NET PAID DATA CELL */}
                      <td className="px-2 py-3 text-center whitespace-nowrap font-black text-emerald-700 bg-emerald-50">
                         ${(user.netUsdtPaid || 0).toFixed(2)}
                      </td>
                    </tr>
                  )
                })
            ...
            {!loading && users.length > 0 && (
              <tfoot className="bg-slate-800 border-t-2 border-slate-900 text-white">
                <tr>
                  <td colSpan="4" className="px-2 py-4 text-right font-black uppercase tracking-widest text-[10px] text-slate-300">
                    Total ({isToday ? 'Today' : isFiltered ? 'Filtered' : 'All-Time'}) :
                  </td>
                  <td className="px-2 py-4 text-center font-black text-indigo-300">${totals.marketWallet.toFixed(2)}</td>
                  <td className="px-2 py-4 text-center font-black text-amber-400">${totals.usdtBep20.toFixed(2)}</td>
                  <td className="px-2 py-4 text-center font-black text-slate-300">${totals.totalIn.toFixed(2)}</td>
                  <td className="px-2 py-4 text-center font-black text-teal-300">${totals.p2p.toFixed(2)}</td>
                  <td className="px-2 py-4 text-center font-black text-blue-300">${totals.deposit.toFixed(2)}</td>
                  <td className="px-2 py-4 text-center font-black text-rose-400">${totals.leaderBonus.toFixed(2)}</td>
                  <td className="px-2 py-4 text-center font-black text-green-300">${totals.fastTrack.toFixed(2)}</td>
                  <td className="px-2 py-4 text-center font-black text-purple-300">${totals.creditToWallet.toFixed(2)}</td>
                  <td className="px-2 py-4 text-center font-black text-red-400">${totals.withdraw.toFixed(2)}</td>
                  
                  {/* 🌟 NEW NET PAID TOTAL FOOTER */}
                  <td className="px-2 py-4 text-center font-black text-emerald-400">${totals.netPaid.toFixed(2)}</td>
                </tr>
              </tfoot>
            )}
          </table>
...
const SummaryCard = ({ label, value, color }) => (
  <div className={`${color} p-3 rounded-xl shadow-sm border flex flex-col justify-center transform hover:scale-[1.02] transition-transform`}>
    <h4 className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest mb-1.5 leading-tight opacity-80">{label}</h4>
    <p className="text-lg sm:text-xl md:text-2xl font-black tracking-tight truncate">{value}</p>
  </div>
);
