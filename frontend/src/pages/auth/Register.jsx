    ...
    <div className="min-h-screen bg-white flex font-sans selection:bg-emerald-500/30">
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center items-center p-4 sm:p-8 relative overflow-y-auto bg-white">
         ...
         <div className="w-full max-w-md animate-in fade-in slide-in-from-bottom-8 duration-700 mt-4 md:mt-0">
           ...
           {/* 🔥 MATCHED HEADER FROM SCREENSHOT */}
           ...
           <div className="mb-6">
             <h2 className="text-4xl font-black text-slate-900 mb-1">Sign Up</h2>
             <p className="text-slate-500 text-sm font-medium">Join the network today.</p>
           </div>

           <form onSubmit={handleRegister} className="space-y-4">
             
             {/* Sponsor Box - Exact match to screenshot */}
             <div className="bg-slate-50/50 p-4 rounded-2xl border border-slate-200 shadow-sm mb-2">
               ...
               <div className="h-4 mt-2 ml-1">
                   {sponsorName && (
                     <p className={`text-[10px] md:text-[11px] font-black tracking-wide uppercase ${sponsorName === 'Invalid Sponsor' ? 'text-red-500' : 'text-emerald-600'}`}>
             ...
             <div className="space-y-3">
                 ...
                 <div className="relative group">
                   ...
                   {country === 'India' && <span className="absolute right-3 top-3.5 text-[9px] md:text-[10px] text-emerald-700 font-black bg-emerald-100 px-2 py-1 rounded-md uppercase">10 Digits</span>}
                 </div>
             </div>
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                <div className="relative group">
                  ...
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-3.5 text-slate-400 hover:text-emerald-500">{showPassword ? <EyeOff size={18} /> : <Eye size={18} />}</button>
                </div>
                ...
                <div className="relative group">
                  ...
                  <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-3 top-3.5 text-slate-400 hover:text-emerald-500">{showConfirmPassword ? <EyeOff size={18} /> : <Eye size={18} />}</button>
                </div>
             </div>

             {errorMsg && (
               <div className="bg-red-50 border border-red-200 text-red-600 text-xs p-3 rounded-xl font-bold flex items-center gap-2 animate-in slide-in-from-top-2 shadow-sm">
                   <XCircle size={16} className="text-red-500" /> {errorMsg}
               </div>
             )}
      ...
      {showPopup && registeredData && (
        <div style={modalOverlay}>
          ...
          <div className="animate-in zoom-in duration-300 relative overflow-hidden" style={modalBox}>
            ...
            <p className="text-slate-500 text-xs uppercase tracking-widest mb-6 font-black">Welcome, {registeredData.name}</p>
            
            <div className="bg-slate-50 border border-slate-200 p-5 rounded-2xl mb-6 text-left relative z-10 shadow-inner">
              <div className="flex justify-between items-center mb-4 pb-4 border-b border-slate-200">
                 <span className="text-slate-400 font-black text-[10px] uppercase tracking-widest">User ID</span>
                 <div className="flex items-center gap-2">
                    <span className="text-emerald-600 font-black text-xl tracking-wider font-mono">{registeredData.userId}</span>
              ...
              <div className="flex justify-between items-center">
                 ...
                 <div className="flex items-center gap-2">
                    <span className="text-slate-900 font-black font-mono bg-white border border-slate-200 px-3 py-1.5 rounded-lg text-sm shadow-sm">{registeredData.password}</span>
                    <button onClick={() => copyToClipboard(registeredData.password)} className="text-slate-400 hover:text-emerald-500 bg-white border border-slate-200 p-1.5 rounded shadow-sm"><Copy size={14}/></button>
                 </div>
              </div>
            </div>

            <div className="bg-amber-50 border border-amber-200 text-amber-600 text-[10px] p-3 rounded-xl font-black mb-6 flex items-center justify-center gap-2 uppercase tracking-widest shadow-sm">
...
const modalBox = { backgroundColor: '#ffffff', border: '1px solid #e2e8f0', padding: '32px 24px', borderRadius: '32px', width: '90%', maxWidth: '400px', textAlign: 'center', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)' };
